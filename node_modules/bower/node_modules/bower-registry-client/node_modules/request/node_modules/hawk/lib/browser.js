...
hawk.client = {
    ...
    header: function (uri, method, options) {
        ...
        if (!uri || (typeof uri !== 'string' && typeof uri !== 'object') || !method || typeof method !== 'string'
        ...
        if (!artifacts.hash &&
            ...
            artifacts.hash = hawk.crypto.calculatePayloadHash(options.payload, credentials.algorithm,
                options.contentType);
        }
    ...
    authenticate: function (request, credentials, artifacts, options) {
        ...
        if (request.getResponseHeader('www-authenticate')) {
            ...
            var attributes = hawk.utils.parseAuthorizationHeader(request.getResponseHeader('www-authenticate'),
                ['ts', 'tsm', 'error']);
            if (!attributes) {
                return false;
            }
        ...
        if (!request.getResponseHeader('server-authorization') && !options.required) {

            return true;
        }

        var attributes = hawk.utils.parseAuthorizationHeader(request.getResponseHeader('server-authorization'),
            ['mac', 'ext', 'hash']);
        if (!attributes) {
            return false;
        }
        ...
        var calculatedHash = hawk.crypto.calculatePayloadHash(options.payload, credentials.algorithm,
    ...
    message: function (host, port, message, options) {
        ...
        if (!host || typeof host !== 'string' || !port || typeof port !== 'number' ||
            message === null || message === undefined || typeof message !== 'string' || !options || typeof options
            !== 'object') {
...
hawk.crypto = {
    ...
    calculateTsMac: function (ts, credentials) {
        ...
        var hash = CryptoJS['Hmac' + credentials.algorithm.toUpperCase()](
            'hawk.' + hawk.crypto.headerVersion + '.ts\n' + ts + '\n', credentials.key);
...
hawk.utils = {
    ...
    parseUri: function (input) {
        ...
        var keys = ['source', 'protocol', 'authority', 'userInfo', 'user', 'password', 'hostname', 'port', 'resource',
            'relative', 'pathname', 'directory', 'file', 'query', 'fragment'];
...
var CryptoJS = CryptoJS || function (h, r) {
        var k = {}, l = k.lib = {}, n = function () {
            ...
            init: function (a, b) {
                ...
                if (c % 4) {
                    for (var e = 0; e < a; e++) {
                        b[c + e >>> 2] |= (d[e >>> 2] >>> 24 - 8 * (e % 4) & 255) << 24 - 8
                            * ((c + e) % 4);
                    }
            ...
            stringify: function (a) {
                ...
                for (var b = a.length, d = [], c = 0; c < b; c += 2) {
                    d[c >>> 3] |= parseInt(a.substr(c, 2), 16) << 24 - 4
                        * (c % 8);
                }
            ...
            stringify: function (a) {
                ...
                for (var b = a.length, d = [], c = 0; c < b; c++) {
                    d[c >>> 2] |= (a.charCodeAt(c) & 255) << 24 - 8 * (c
                        % 4);
                }
            ...
            reset: function () {
                ...
                var b = this._data, d = b.words, c = b.sigBytes, e = this.blockSize, f = c / (4 * e), f = a ? h.ceil(f)
    ...
    var k = CryptoJS, b = k.lib, m = b.WordArray, l = b.Hasher, d = [], b = k.algo.SHA1 = l.extend({
        _doReset: function () {
            ...
            for (var a = this._hash.words, e = a[0], f = a[1], h = a[2], j = a[3], b = a[4], c = 0; 80 > c; c++) {
                ...
                g = (e << 5 | e >>> 27) + b + d[c];
                g = 20 > c ? g + ((f & h | ~f & j) + 1518500249) : 40 > c ? g + ((f ^ h ^ j) + 1859775393) : 60 > c ? g
                            + ((f & h | f & j | h & j) - 1894007588) : g + ((f ^ h ^ j) - 899497514);
                b = j;
                j = h;
                h = f << 30 | f >>> 2;
                f = e;
        ...
        _doReset: function () {
            ...
            for (var a = this._hash.words, c = a[0], d = a[1], b = a[2], k = a[3], f = a[4], g = a[5], j = a[6], l = a[7], e = 0;
                ...
                if (16 > e) {
                    ...
                    n[e] = ((m << 25 | m >>> 7) ^ (m << 14 | m >>> 18) ^ m >>> 3) + n[e - 7] + ((p << 15 | p >>> 17)
                        ^ (p << 13 | p >>> 19) ^ p >>> 10) + n[e - 16]
                }
                m = l + ((f << 26 | f >>> 6) ^ (f << 21 | f >>> 11) ^ (f << 7 | f >>> 25)) + (f & g ^ ~f & j) + t[e]
                    + n[e];
...
(function () {
    ...
    c.algo.HMAC = c.lib.Base.extend({
        init: function (a, b) {
            ...
            b.clamp();
            for (var f = this._oKey = b.clone(), g = this._iKey = b.clone(), h = f.words, j = g.words, d = 0; d < c;
                d++) {
...
(function () {
    ...
    h.enc.Base64 = {
        stringify: function (b) {
            ...
            b = [];
            for (var a = 0; a < f; a += 3) {
                for (var d = (e[a >>> 2] >>> 24 - 8 * (a % 4) & 255) << 16 | (e[a + 1 >>> 2]
                    >>> 24 - 8 * ((a + 1) % 4) & 255) << 8 | e[a + 2 >>> 2] >>> 24 - 8 * ((a + 2) % 4) & 255, g = 0;
                    4 > g && a + 0.75 * g < f; g++) {
