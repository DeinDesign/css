...
function UpdateNotifier(options) {
    ...
    if (!options.packageName || !options.packageVersion) {
        this.packageFile = require(
            path.resolve(path.dirname(module.parent.filename), options.packagePath || 'package'));
    }
    ...
    this.updateCheckInterval = typeof options.updateCheckInterval === 'number' ? options.updateCheckInterval : 1000 * 60
        * 60 * 24; // 1 day
...
UpdateNotifier.prototype.checkNpm = function (cb) {
    ...
    request({url: url, json: true, timeout: this.updateCheckTimeout, proxy: proxyServer},
