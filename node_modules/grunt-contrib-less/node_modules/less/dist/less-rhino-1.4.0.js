    ...
    return parser = {
            ...
            root.toCSS = (function (evaluate) {
                ...
                return function (options, variables) {
                    ...
                    if ((importError = parser.imports.error)) { // Check if there was an error during importing
                        if (importError instanceof LessError) {
                            throw importError;
                        } else {
                            throw new (LessError)(importError, env);
                        }
                    }
        ...
        parsers: {
            ...
            mixin: {
                ...
                //
                call: function () {
                    var elements = [], e, c, argsSemiColon = [], argsComma = [], args, delim, arg, nameLoop, expressions, isSemiColonSeperated, expressionContainsNamed, index = i, s = input.charAt(
                        i), name, value, important = false;
            ...
            selector: function () {
                ...
                if (elements.length > 0) {
                    return new (tree.Selector)(elements, extend)
                }
                if (extend) {
                    error("Extend must be used to extend a selector");
                }
            },
            attribute: function () {
                var attr = '', key, val, op;
...
(function (tree) {
    ...
    tree.functions = {
        ...
        contrast: function (color, dark, light, threshold) {
            ...
            if (((0.2126 * (color.rgb[0] / 255) + 0.7152 * (color.rgb[1] / 255) + 0.0722 * (color.rgb[2] / 255))
...
(function (tree) {
                ...
                if ((mixins = env.frames[i].find(this.selector)).length > 0) {
                    ...
                    for (m = 0; m < mixins.length; m++) {
                        ...
                        for (f = 0; f < env.frames.length; f++) {
                            if ((!(mixin instanceof tree.mixin.Definition)) && mixin === (env.frames[f].originalRuleset
                                || env.frames[f])) {
                                isRecursive = true;
                                break;
                            }
    ...
    tree.mixin.Definition.prototype = {
        ...
        evalParams: function (env, mixinEnv, args, evaldArguments) {
            var frame = new (tree.Ruleset)(null, []), varargs, arg, params = this.params.slice(
                0), i, j, val, name, isNamedFound, argIndex;
        ...
        matchCondition: function (args, env) {
            if (this.condition && !this.condition.eval({
                    frames: [this.evalParams(env, {frames: this.frames.concat(env.frames)}, args, [])].concat(
                        env.frames)
                })) {
                return false
            }
