...
//
        ...
        var imports = this.imports = {
            ...
            push: function (path, currentFileInfo, callback) {
                ...
                less.Parser.importer(path, currentFileInfo, function (e, root, fullPath) {
                    ...
                    if (e && !parserImporter.error) {
                        parserImporter.error = e;
                    }
            ...
            parsers: {
                ...
                entities: {
                    ...
                    call: function () {
                        ...
                        if (!(name = /^([\w-]+|%|progid:[\w\.]+)\(/.exec(chunks[j]))) {
                            return;
                        }
                ...
                selector: function () {
                    ...
                    if (elements.length > 0) {
                        return new (tree.Selector)(elements, extendList);
                    }
                    if (extendList.length) {
                    ...
                    do {
                        if (e = $(this.entities.keyword)) {
                            ...
                            if ($(')')) {
                                if (p && e) {
                                    nodes.push(
                                        new (tree.Paren)(new (tree.Rule)(p, e, null, i, env.currentFileInfo, true)));
                                } else if (e) {
    ...
    (function (tree) {
        ...
        tree.functions = {
            ...
            escape: function (str) {
                return new (tree.Anonymous)(encodeURI(str.value).replace(/=/g, "%3D").replace(/:/g, "%3A").replace(/#/g,
                    "%23").replace(/;/g, "%3B").replace(/\(/g, "%28").replace(/\)/g, "%29"));
            },
            ...
            overlay: function (color1, color2) {
                var r = color1.rgb[0] < 128 ? 2 * color1.rgb[0] * color2.rgb[0] / 255 : 255 - 2 * (255 - color1.rgb[0])
                    * (255 - color2.rgb[0]) / 255;
                var g = color1.rgb[1] < 128 ? 2 * color1.rgb[1] * color2.rgb[1] / 255 : 255 - 2 * (255 - color1.rgb[1])
                    * (255 - color2.rgb[1]) / 255;
                var b = color1.rgb[2] < 128 ? 2 * color1.rgb[2] * color2.rgb[2] / 255 : 255 - 2 * (255 - color1.rgb[2])
                    * (255 - color2.rgb[2]) / 255;
                return this.rgb(r, g, b);
            },
            ...
            hardlight: function (color1, color2) {
                var r = color2.rgb[0] < 128 ? 2 * color2.rgb[0] * color1.rgb[0] / 255 : 255 - 2 * (255 - color2.rgb[0])
                    * (255 - color1.rgb[0]) / 255;
                var g = color2.rgb[1] < 128 ? 2 * color2.rgb[1] * color1.rgb[1] / 255 : 255 - 2 * (255 - color2.rgb[1])
                    * (255 - color1.rgb[1]) / 255;
                var b = color2.rgb[2] < 128 ? 2 * color2.rgb[2] * color1.rgb[2] / 255 : 255 - 2 * (255 - color2.rgb[2])
                    * (255 - color1.rgb[2]) / 255;
                return this.rgb(r, g, b);
            },
                    ...
                    if (this.env.ieCompat !== false) {
                        if (!this.env.silent) {
                            console.warn(
                                ...
                                filePath, fileSizeInKB, DATA_URI_MAX_KB);
                        }

                        return new tree.URL(filePathNode || mimetypeNode, this.currentFileInfo).eval(this.env);
                    } else if (!this.env.silent) {
                        // if explicitly disabled (via --no-ie-compat on CLI, or env.ieCompat === false), merely warn
                        console.warn("WARNING: Embedding %s (%dKB) exceeds IE8's data-uri size limit of %dKB!",
                            filePath, fileSizeInKB, DATA_URI_MAX_KB);
                    }
                }
                ...
                if (op === '+' || op === '-') {
                    if (unit.numerator.length === 0 && unit.denominator.length === 0) {
                        ...
                        if (env.strictUnits && other.unit.toString() !== unit.toString()) {
                            throw new Error("Incompatible units. Change the units or use the unit function. Bad units: '"
                                + unit.toString() +
...
//
        tree.Import.prototype = {
            ...
            evalForImport: function (env) {
                return new (tree.Import)(this.path.eval(
                    env), this.features, this.options, this.index, this.currentFileInfo);
            },
                    ...
                    if ((mixins = env.frames[i].find(this.selector)).length > 0) {
                        ...
                        for (m = 0; m < mixins.length; m++) {
                            ...
                            for (f = 0; f < env.frames.length; f++) {
                                if ((!(mixin instanceof tree.mixin.Definition)) && mixin
                                    === (env.frames[f].originalRuleset || env.frames[f])) {
                                    isRecursive = true;
                                    break;
                                }
                            }
        ...
        tree.Ruleset.prototype = {
                        ...
                        if (rule.name === "@charset") {
                            ...
                            if (env.charset) {
                                if (rule.debugInfo) {
                                    ...
                                    rulesets.push(
                                        new tree.Comment("/* " + cssValue.replace(/\n/g, "") + " */\n").toCSS(env));
                                }
                                continue;
                            }
                            env.charset = true;
                            ...
                            if (context.length == 0) {
                                ...
                                for (k = 0; k < context.length; k++) {
                                    ...
                                    if (sel.length > 0) {
                                        ...
                                        newJoinedSelector = new (tree.Selector)(lastSelector.elements.slice(
                                            0), selector.extendList);
                                    ...
                                    if (parentSel.length > 0) {
                                        ...
                                        newJoinedSelector.elements.push(
                                            new (tree.Element)(el.combinator, parentSel[0].elements[0].value, 0));
                                        newJoinedSelector.elements = newJoinedSelector.elements.concat(
                                            parentSel[0].elements.slice(1));
                                    }
            ...
            mergeElementsOnToSelectors: function (elements, selectors) {
                ...
                for (i = 0; i < selectors.length; i++) {
                    ...
                    if (sel.length > 0) {
                        sel[sel.length - 1] = new (tree.Selector)(sel[sel.length - 1].elements.concat(
                            elements), sel[sel.length - 1].extendList);
                    }
                    else {
    ...
    (function (tree) {
        ...
        tree.Variable = function (name, index, currentFileInfo) {
            this.name = name, this.index = index, this.currentFileInfo = currentFileInfo
        };
            ...
            visitImport: function (importNode, visitArgs) {
                ...
                if (!importNode.css) {
                    ...
                    if (evaldImportNode && !evaldImportNode.css) {
                        ...
                        this._importer.push(importNode.getPath(), importNode.currentFileInfo,
                            function (e, root, imported) {
                                if (e && !e.filename) {
                                    e.index = importNode.index;
                                    e.filename = importNode.currentFileInfo.filename;
                                }
                                if (imported && !importNode.options.multiple) {
    ...
    (function (tree) {
        ...
        tree.extendFinderVisitor.prototype = {
            ...
            visitRuleset: function (rulesetNode, visitArgs) {
                ...
                for (i = 0; i < rulesetNode.paths.length; i++) {
                    ...
                    extendList = selector.extendList.slice(0).concat(allSelectorsExtendList).map(
                        function (allSelectorsExtend) {
        ...
        tree.processExtendsVisitor.prototype = {
            ...
            visitRuleset: function (rulesetNode, visitArgs) {
                ...
                var matches, pathIndex, extendIndex, allExtends = this.allExtendsStack[this.allExtendsStack.length
                - 1], selectorsToAdd = [], extendVisitor = this, selectorPath;
                ...
                for (extendIndex = 0; extendIndex < allExtends.length; extendIndex++) {
                    for (pathIndex = 0; pathIndex < rulesetNode.paths.length; pathIndex++) {
                        ...
                        if (selectorPath[selectorPath.length - 1].extendList.length) {
                            continue;
                        }
            ...
            findMatch: function (extend, haystackSelectorPath) {
                ...
                for (haystackSelectorIndex = 0; haystackSelectorIndex < haystackSelectorPath.length;
                    haystackSelectorIndex++) {
                    hackstackSelector = haystackSelectorPath[haystackSelectorIndex];

                    for (hackstackElementIndex = 0; hackstackElementIndex < hackstackSelector.elements.length;
                        ...
                        for (i = 0; i < potentialMatches.length; i++) {
                            ...
                            if (!extendVisitor.isElementValuesEqual(needleElements[potentialMatch.matched].value,
                                    haystackElement.value) ||
                                (potentialMatch.matched > 0 && needleElements[potentialMatch.matched].combinator.value
                                !== targetCombinator)) {
                                potentialMatch = null;
                            } else {
                            ...
                            if (potentialMatch) {
                                ...
                                if (potentialMatch.finished &&
                                    (!extend.allowAfter && (hackstackElementIndex + 1
                                    < hackstackSelector.elements.length || haystackSelectorIndex + 1
            ...
            visitDirective: function (directiveNode, visitArgs) {
                var newAllExtends = directiveNode.allExtends.concat(
...
//
    ...
    function loadStyleSheet(sheet, callback, reload, remaining) {
        ...
        if (env.relativeUrls) {
            ...
            if (less.rootpath) {
                newFileInfo.rootpath = extractUrlParts(
                    less.rootpath + pathDiff(hrefParts.path, newFileInfo.entryPath)).path;
            } else {
                    ...
                    new (less.Parser)(env).parse(data, function (e, root) {
                        ...
                        try {
                            callback(e, root, data, sheet,
                                {local: false, lastModified: lastModified, remaining: remaining}, href);
