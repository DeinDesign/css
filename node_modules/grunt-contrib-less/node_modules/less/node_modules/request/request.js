...
Request.prototype.init = function (options) {
    ...
    if (!self.hasHeader('host')) {
        ...
        if (self.uri.port) {
            if (!(self.uri.port === 80 && self.uri.protocol === 'http:') && !(self.uri.port === 443 && self.uri.protocol
                === 'https:')) {
...
Request.prototype.onResponse = function (response) {
    ...
    if (self.setHost && self.hasHeader('host')) {
        delete self.headers[self.hasHeader('host')]
    }
    if (self.timeout && self.timeoutTimer) {
        clearTimeout(self.timeoutTimer)
        self.timeoutTimer = null
    ...
    if (response.statusCode >= 300 && response.statusCode < 400 && hasHeader('location', response.headers)) {
        ...
        switch (authVerb) {
            ...
            case 'Digest':
                ...
                var digestResponse = qop ? md5(
                        ha1 + ':' + challenge.nonce + ':' + nc + ':' + cnonce + ':' + qop + ':' + ha2) : md5(
    ...
    if (redirectTo) {
        ...
        if (response.statusCode != 401) {
            ...
            if (self.headers) {
                ...
                if (self.hasHeader('content-type')) {
                    delete self.headers[self.hasHeader('content-type')]
                }
                if (self.hasHeader('content-length')) {
                    delete self.headers[self.hasHeader('content-length')]
                }
            }
        }
...
Request.prototype.setHeader = function (name, value, clobber) {
    ...
    if (clobber || !this.hasHeader(name)) {
        this.headers[name] = value
    } else {
        this.headers[this.hasHeader(name)] += ',' + value
    }
    return this
}
