...
// localVar: 1, evaled: 1
    ...
    <p>Gets/sets the group identity of the process. (See setgid(2).) This is the numerical group id, not the group
      name.</p>
...
sys.puts(sys.inspect(sys, true, null));
    ...
    <p>Example of running <code>ls -lh /usr</code>, capturing <code>stdout</code>, <code>stderr</code>, and the exit
      code:</p>
...
, killSignal: 'SIGKILL'
    ...
    <p>Similar to <code>process.compile</code>. <code>Script.runInThisContext</code> compiles <code>code</code> as if it
      were loaded from <code>filename</code>,
      runs it and returns the result. Running code does not have access to local scope. <code>filename</code> is
      optional.</p>
...
// localVar: 1, evaled: 1
    ...
    <p><code>Script.runInThisContext</code> does not have access to the local scope, so <code>localVar</code> is
      unchanged.
      <code>eval</code> does have access to the local scope, so <code>localVar</code> is changed.</p>

    <p>In case of syntax error in <code>code</code>, <code>Script.runInThisContext</code> emits the syntax error to
      stderr
      and throws.an exception.</p>

    <h3>Script.runInNewContext(code, sandbox, filename)</h3>

    <p><code>Script.runInNewContext</code> compiles <code>code</code> to run in <code>sandbox</code> as if it were
...
// { animal: 'cat', count: 3, name: 'kitty' }
    ...
    <p>Similar to <code>Script.runInThisContext</code> (note capital 'S'), but now being a method of a precompiled
      Script object.
...
// 1000
    ...
    <p>Similar to <code>Script.runInNewContext</code> (note capital 'S'), but now being a method of a precompiled Script
      object.
      <code>script.runInNewContext</code> runs the code of <code>script</code> with <code>sandbox</code> as the global
...
fs.stat('/tmp/world', function (err, stats) {
  ...
  fs.stat('/tmp/world', function (err, stats) {
    ...
    <p>Asynchronous stat(2), lstat(2) or fstat(2). The callback gets two arguments <code>(err, stats)</code> where
...
, 'accept': '*/*'
    ...
    <p>Enables HTTPS support for the server, with the crypto module credentials specifying the private key and
      certificate of the server, and optionally the CA certificates for use in client authentication.</p>

    <p>If the credentials hold one or more CA certificates, then the server will request for the client to submit a
      client certificate as part of the HTTPS connection handshake. The validity and content of this can be accessed via
      verifyPeer() and getPeerCertificate() from the server's request.connection.</p>
...
request.addListener('response', function (response) {
  ...
  response.addListener('data', function (chunk) {
    ...
    <p><code>secure</code> is an optional boolean flag to enable https support and <code>credentials</code> is an
      optional credentials object from the crypto module, which may hold the client's private key, certificate, and a
      list of trusted CA certificates.</p>

    <p>If the connection is secure, but no explicit CA certificates are passed in the credentials, then node.js will
      default to the publicly trusted list of CA certificates, as given in
    ...
    <p>Returns true or false depending on the validity of the server's certificate in the context of the defined or
      default list of trusted CA certificates.</p>

    <h3>client.getPeerCertificate()</h3>

    <p>Returns a JSON structure detailing the server's certificate, containing a dictionary with keys for the
      certificate 'subject', 'issuer', 'valid_from' and 'valid_to'</p>
...
server.listen(8124, 'localhost');
    ...
    <p>Either <code>'closed'</code>, <code>'open'</code>, <code>'opening'</code>, <code>'readOnly'</code>, or <code>'writeOnly'</code>.
    </p>
    ...
    <p>Enables HTTPS support for the stream, with the crypto module credentials specifying the private key and
      certificate of the stream, and optionally the CA certificates for use in peer authentication.</p>

    <p>If the credentials hold one ore more CA certificates, then the stream will request for the peer to submit a
      client certificate as part of the HTTPS connection handshake. The validity and content of this can be accessed via
      verifyPeer() and getPeerCertificate().</p>

    <h3>stream.verifyPeer()</h3>

    <p>Returns true or false depending on the validity of the peers's certificate in the context of the defined or
      default list of trusted CA certificates.</p>

    <h3>stream.getPeerCertificate()</h3>

    <p>Returns a JSON structure detailing the peer's certificate, containing a dictionary with keys for the certificate
      'subject', 'issuer', 'valid_from' and 'valid_to'</p>
    ...
    <p>The crypto module requires OpenSSL to be available on the underlying platform. It offers a way of encapsulating
      secure credentials to be used as part of a secure HTTPS net or http connection.</p>
    ...
    <p>Creates and returns a hash object, a cryptographic hash with the given algorithm which can be used to generate
      hash digests.</p>

    <p><code>algorithm</code> is dependent on the available algorithms supported by the version of OpenSSL on the
      platform. Examples are sha1, md5, sha256, sha512, etc. On recent releases, <code>openssl
        list-message-digest-algorithms</code> will display the available digest algorithms.</p>

    <h3>hash.update(data)</h3>

    <p>Updates the hash content with the given <code>data</code>. This can be called many times with new data as it is
      streamed.</p>

    <h3>hash.digest(encoding)</h3>

    <p>Calculates the digest of all of the passed data to be hashed. The <code>encoding</code> can be 'hex', 'binary' or
      'base64'.</p>
    ...
    <p>Update the hmac content with the given <code>data</code>. This can be called many times with new data as it is
      streamed.</p>

    <h3>hmac.digest(encoding)</h3>

    <p>Calculates the digest of all of the passed data to the hmac. The <code>encoding</code> can be 'hex', 'binary' or
      'base64'.</p>
    ...
    <p><code>algorithm</code> is dependent on OpenSSL, examples are aes192, etc. On recent releases, <code>openssl
    ...
    <p>Updates the cipher with <code>data</code>, the encoding of which is given in <code>input_encoding</code> and can
      be 'utf8', 'ascii' or 'binary'. The <code>output_encoding</code> specifies the output format of the enciphered
      data, and can be 'binary', 'base64' or 'hex'.</p>
    ...
    <p>Creates and returns a decipher object, with the given algorithm and key. This is the mirror of the cipher object
      above.</p>

    <h3>decipher.update(data, input_encoding, output_encoding)</h3>

    <p>Updates the decipher with <code>data</code>, which is encoded in 'binary', 'base64' or 'hex'. The <code>output_decoding</code>
      specifies in what format to return the deciphered plaintext - either 'binary', 'ascii' or 'utf8'.</p>
    ...
    <p>Creates and returns a signing object, with the given algorithm. On recent OpenSSL releases, <code>openssl
      list-public-key-algorithms</code> will display the available signing algorithms. Examples are 'RSA-SHA256'.</p>
    ...
    <p>Calculates the signature on all the updated data passed through the signer. <code>private_key</code> is a string
    ...
    <p>Creates and returns a verification object, with the given algorithm. This is the mirror of the signing object
      above.</p>
    ...
    <p>Verifies the signed data by using the <code>public_key</code> which is a string containing the PEM encoded public
      key, and <code>signature</code>, which is the previously calculates signature for the data, in the <code>signature_format</code>
...
dns.resolve4('www.google.com', function (err, addresses) {
  ...
  for (var i = 0; i &lt; addresses.length; i++) {
    ...
    <p>Tests if value is a <code>true</code> value, it is equivalent to <code>assert.equal(true, value, message);</code>
    </p>
    ...
    <p>Tests if value is not a false value, throws if it is a true value. Useful when testing the first argument, <code>error</code>
      in callbacks.</p>
...
''
    ...
    <p>Test whether or not the given path exists. Then, call the <code>callback</code> argument with either true or
      false. Example:</p>
  ...
  sys.debug(exists ? "it's there" : "no passwd!");
    ...
    <ul>
      ...
      <li><p><code>pathname</code></p>
        ...
        <p>The path section of the URL, that comes after the host and before the query, including the initial slash if
          present. Example: <code>'/p/a/t/h'</code></p></li>
      <li><p><code>search</code></p>

        <p>The 'query string' portion of the URL, including the leading question mark. Example:
...
'foo=bar&amp;foo=baz&amp;foo=boz'
    ...
    <p>Note that when <code>munge</code> is <code>false</code>, parameter names with object values will still be munged.
    </p>
...
, 'b': 'c'
    ...
    <p>The escape function used by <code>querystring.stringify</code>, provided so that it could be overridden if
      necessary.</p>

    <h3>querystring.unescape</h3>

    <p>The unescape function used by <code>querystring.parse</code>, provided so that it could be overridden if
      necessary.</p>
...
'message'
    ...
    <ul>
      ...
      <li><p><code>.clear</code> - Resets the <code>scope</code> object to an empty object and clears any multi-line
        expression.</p></li>
...
def build(bld):
  ...
  obj.source = 'hello.cc'
    ...
    <p><code>node-waf</code> is just http://code.google.com/p/waf/[WAF], the python-based build system.
      <code>node-waf</code> is
