window.onload = function () { // wait for load in a dumb way because B-0
    ...
    function createGist(configData) {
        ...
        .success(function (result) {
            history.replaceState(false, document.title,
                window.location.origin + window.location.pathname + '?id=' + result.id)
        })
        .error(function (err) {
    ...
    function generateZip(css, js, fonts, complete) {
        if (!css && !js) {
            return showError('<strong>Ruh roh!</strong> No Bootstrap files selected.',
